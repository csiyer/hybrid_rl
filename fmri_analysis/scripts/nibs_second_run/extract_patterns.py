...
preproc_base = os.path.join(BIDSDIR, 'derivatives','fmriprep')
    ...
    for roi in ['hipp','vmpfc','a1']:
        ...
        for run in range(1,6):
            for trial_type in ['choice','fb']:
                beta_file = os.path.join(subdir, f'{sub_id}_task-main_run-{run}_space-MNI152NLin2009cAsym_desc-{trial_type}_betaseries.nii.gz')
...
if __name__ == "__main__":
    ...
    for roi in ['hipp','vmpfc','a1']:
        ...
        for sub_num in range(1,32):
            ...
            sub_patterns_dict_path = os.path.join(subdir, f"{roi}_patterns_{sub}.pkl")
